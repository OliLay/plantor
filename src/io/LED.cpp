...
void OnePinLED::displayColor(uint8_t r, uint8_t g, uint8_t b) {
    // this is a non RGB-LED -> we can just turn it on
    turnOn();
}
...
void RGBLED::displayColor(uint8_t r, uint8_t g, uint8_t b) {
    turnOff();
    redLed.turnOn(r);
    greenLed.turnOn(g);
    blueLed.turnOn(b);
